...
export default function ActivationGate() {
  ...
  useEffect(() => {
    ...
    checkDevice(deviceId); // first check immediately

    // 🔄 keep checking every 2 seconds until active
  ...
  if (status.active)
    return <SignageBoard deviceId={localStorage.getItem('signage_device_id')} />;
    ...
    <div style={{ position: 'relative', minHeight: '100vh' }}>
      ...
      >
        ...
        >
          ...
          <p>
            This device is not activated. Show this activation code in your admin panel to
            activate the device.
          </p>
          <p>
            <strong>Device ID:</strong> {localStorage.getItem('signage_device_id')}
          </p>
          <p>
            <strong>Activation key:</strong>{' '}
            <code style={{ fontSize: 18, letterSpacing: 2 }}>{activationKey}</code>
          </p>
          <p>Waiting for activation... (This page will auto-refresh when activated)</p>
        </div>
      </div>
      {/* Render SignageBoard underneath (blurred) */}
